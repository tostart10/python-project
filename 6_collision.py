...
while running:
    ...
    character_rect.left = character_x_pos  #실제로 화면상에서 위치하고있는 캐릭터의 rect 정보를 업데이트 해주는 코드
    character_rect.top = character_y_pos

    enemy_rect = enemy.get_rect()
    enemy_rect.left = enemy_x_pos   #enemy_rect의 위치를 반영하는 코드
    enemy_rect.top = enemy_y_pos

    # 충돌 체크
    if character_rect.colliderect(enemy_rect):  #colliderect는 rect(사각형)을 기준으로 충돌이 있었는지를 확인하는 함수
        print("충돌했어요")                       #character_rect가 enemy_rect와 충돌을 했느냐라는 의미의 코드 , 맞으면if구문을 탄다
        running = False        #while 문에서 계속 돌고 있던 게임을 종료하게하는 코드
